...
async function fetchAll() {
  ...
  try {
    ...
    const taskResults = await Promise.allSettled(
      lists.value.map(l => getTasks(l.listId, { pageSize: 200 }))
    )
    lists.value.forEach((l, idx) => {
      if (taskResults[idx].status === 'fulfilled') {
        map[l.listId] = taskResults[idx].value.data?.records || []
      } else {
        map[l.listId] = []
      }
    })
    ...
    if (resData?.msg) {
      errorMsg.value = resData.msg
    } else if (err?.response?.status === 401 || resData?.code === 401) {
      errorMsg.value = '请先登录后再查看项目'
    } else if (err?.response?.status === 403 || resData?.code === 403) {
      errorMsg.value = '您不是该项目的团队成员，无权查看'
    } else {
      errorMsg.value = '项目加载失败，请检查后端服务是否正常运行'
    }
...
async function handleCreateList() {
  ...
  try {
    await createTaskList(projectId.value, { name: newListName.value.trim() })
      ...
      <el-card class="section-card" v-if="stages.length">
        ...
        <div class="stages-row">
          ...
          >
            ...
            <el-select
              :model-value="stage.status" size="small"
              @change="(v) => handleStatusChange(stage.stageId, v)"
      ...
      <el-dialog v-model="taskDialogVisible" :title="taskFormMode==='create'?'创建任务':'编辑任务'" width="500px">
        <el-form>
          ...
          <el-form-item label="执行人">
            <el-select v-model="taskForm.assigneeId" placeholder="可选" clearable style="width:100%">
